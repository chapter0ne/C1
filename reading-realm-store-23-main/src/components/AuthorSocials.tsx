...
const AuthorSocials: React.FC<AuthorSocialsProps> = ({ authorSocials, className = "" }) => {
  ...
  if (!authorSocials || (!authorSocials.instagram && !authorSocials.twitter && !authorSocials.tiktok)) {
    console.log('❌ No social media data provided or all empty');
    return null;
  }
  
  console.log('✅ Social media data found, rendering buttons');

  const socialLinks = [
    {
      platform: 'instagram',
      username: authorSocials.instagram,
      url: `https://instagram.com/${authorSocials.instagram}`,
      icon: (
    ...
    {
      platform: 'twitter',
      username: authorSocials.twitter,
      url: `https://x.com/${authorSocials.twitter}`,
      icon: (
        <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
    ...
    {
      platform: 'tiktok',
      username: authorSocials.tiktok,
      url: `https://tiktok.com/@${authorSocials.tiktok}`,
      icon: (
        <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
